...
void test_0x69_adc_immediate_carry_overflow()
{
    CPU cpu = make_cpu();
    ...
    printf("PASSED: test_0x69_and_immediate_carry_overflow\n");
}

void test_0x69_adc_immediate_carry_through()
{
    CPU cpu = make_cpu();
    uchar program[9] = {0xA9, 0xFF, 0x69, 0x02, 0xA9, 0x02, 0x69, 0x02, 0x00}; // (256 + 2) carries to (2 + 2)
...
{
    CPU cpu = make_cpu();
    uchar program[4] = {0xA9, 0x02, 0x0A, 0x00}; 
    cpu_load_and_run(&cpu, program, 4);
    assert(cpu.reg_a == 0x04); 
    assert((cpu.status & 0b00000010) == 0); // Zero
    assert((cpu.status & 0b10000000) == 0); // Negative
    assert((cpu.status & 0b00000001) == 0); // Carry
    assert((cpu.status & 0b01000000) == 0); // Overflow
    printf("PASSED: test_0x0A_asl_accumulator\n");	
}

void test_0x0a_asl_accumulator_carry()
{
    CPU cpu = make_cpu();
    uchar program[6] = {0xA9, 0x84, 0x0A, 0x69, 0x02, 0x00}; 
    cpu_load_and_run(&cpu, program, 6);
    assert(cpu.reg_a == 0x0b); 
    assert((cpu.status & 0b00000010) == 0); // Zero
    assert((cpu.status & 0b10000000) == 0); // Negative
    assert((cpu.status & 0b00000001) == 0); // Carry
    assert((cpu.status & 0b01000000) == 0); // Overflow
    printf("PASSED: test_0x0A_asl_accumulator_carry\n");	
}
